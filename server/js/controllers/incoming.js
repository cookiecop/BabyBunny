...
module.exports = Incoming = cls.Class.extend({
    ...
    handleIntro: function(message) {
        ...
        if (isRegistering) {
            //Email check
	    self.mysql.getMySQLQuery().query('SELECT * FROM `player_data` WHERE `player_data`.`email`= ?', [self.player.email], function(error, rows, fields) {
	        var exists = false;
	        _.each(rows, function(row) {
	            if (row.email === self.player.email)
	                exists = true;
	        });
	        if(exists) {
            ...
            //Username check
            self.mysql.getMySQLQuery().query('SELECT * FROM `player_data` WHERE `player_data`.`username`= ?', [self.player.username], function(error, rows, fields) {
	        var exists = false;
	        _.each(rows, function(row) {
	            if (row.username === self.player.username)
	                exists = true;
	        });
	        if(exists) {
            ...
            //Register to game
            self.mysql.getMySQLQuery().query('SELECT * FROM `player_data` WHERE `player_data`.`username`= ? OR `player_data`.`email`= ? ', [self.player.username, self.player.email], function(error, rows, fields) {
	        var exists = false;
	        _.each(rows, function(row) {
	            exists = true;
	        });
            ...
            //Username check
            self.mysql.getMySQLQuery().query('SELECT * FROM `player_data` WHERE `player_data`.`username`= ? AND `player_data`.`password`= ? ', [self.player.username, self.player.password], function(error, rows, fields) {
                var exists = false;
                _.each(rows, function(row) {
                    if (row.username === self.player.username)
                        exists = true;
